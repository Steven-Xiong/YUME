...
def distill_one_step(
    ...
    for _ in range(gradient_accumulation_steps):
        ...
        MAX_PIXEL_FRAMES = 129
        if pixel_values_vid.shape[1] > MAX_PIXEL_FRAMES:
            pixel_values_vid = pixel_values_vid[:, -MAX_PIXEL_FRAMES:]
        ...
        latent_model_input, timestep, arg_c, noise, model_input, _ , arg_null= wan_i2v.generate_for_train(
            model_input,
            device,
            caption,
            img,
        ...
        wandb_log({"train/flow_loss": loss.item()}, step=step)
        ...
        if Distil:
            ...
            wandb_log({
                "train/disc_loss": loss_d.item(),
                "train/gan_loss": gan_loss.item(),
                "train/disc_grad_norm": d_grad_norm,
            }, step=step)
    ...
    if step % validation_steps == 0:
            ...
            with torch.autocast("cuda", dtype=torch.bfloat16):
                for i in range(50):
                    ...
                    noise_pred_cond = transformer(\
                        latent_model_input, t=timestep, flag=False, **arg_c)
                    if isinstance(noise_pred_cond, (list, tuple)):
                        noise_pred_cond = noise_pred_cond[0]
                    noise_pred_uncond = transformer(\
                        latent_model_input, t=timestep, flag=False, **arg_null)
                    if isinstance(noise_pred_uncond, (list, tuple)):
                        noise_pred_uncond = noise_pred_uncond[0]

                    noise_pred_cond = noise_pred_uncond + 5.0*(noise_pred_cond - noise_pred_uncond)
                    _latent_frames = int(latent.shape[1])
                    _pred_frames = int(noise_pred_cond.shape[1])
                    _tail_len = min(9, _latent_frames, _pred_frames)
                    _latent_tail = latent[:, -_tail_len:, :, :]
                    _pred_tail = noise_pred_cond[:, -_tail_len:, :, :]

                    if i+1 == 50:
                        temp_x0 = _latent_tail + (0-sampling_sigmas[i]) * _pred_tail
                    else:
                        temp_x0 = _latent_tail + (sampling_sigmas[i+1]-sampling_sigmas[i]) * _pred_tail

                    latent = torch.cat(
                        [
                            noise[:, :-_tail_len, :, :] * sampling_sigmas[max(i - 1, 0)] +
                            (1 - sampling_sigmas[max(i - 1, 0)]) * model_input[:, :-_tail_len, :, :],
                            temp_x0
                        ],
                        dim=1
                    )
        ...
        _base_dir = sample_output_dir if sample_output_dir is not None else "./generated_test_video"
        i2v_dir = os.path.join(_base_dir, "i2v")
        ...
        if rand_num_img_flag < 0.4:
            ...
            export_to_video(video[0] , gen_filename, fps=16)
            ori_filename = os.path.join(
                i2v_dir,
                videoid_str+"_"+"_imgori_"+str(device)+".mp4",
            )
            export_to_video(video_ori[0] , ori_filename, fps=16)
        else:
            gen_filename = os.path.join(
                i2v_dir,
            ...
            ori_filename = os.path.join(
                i2v_dir,
                videoid_str+"_ori_"+str(device)+".mp4",
            )
            export_to_video(video_ori[0] , ori_filename, fps=16)
        if dist.get_rank() == 0 and wandb is not None and wandb.run is not None:
            try:
                wandb.log({
                    "samples/generated_video": wandb.Video(gen_filename, fps=16, format="mp4"),
                    "samples/original_video": wandb.Video(ori_filename, fps=16, format="mp4"),
                }, step=step)
            except Exception:
                pass
...
def main(args):
    ...
    # wan23.Yume does not have init_model() - all initialization done in __init__
    # wan_i2v.init_model(
    #     config=cfg,
    #     checkpoint_dir=ckpt_dir,
    #     device_id=device,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Base model checkpoint directory containing T5, VAE, and initial DiT weights. "
             "For Stage 2/3, point this to the Stage 1 output dir if it contains the full "
             "model structure, or keep pointing to the original pretrained dir and use "
             "--resume_from_checkpoint to load the Stage 1 DiT weights on top.",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Directory for training-step preview videos. Defaults to <output_dir>/samples.",
    )
    parser.add_argument(
        "--full_mp4",
        type=str,
    ...
    parser.add_argument("--wandb_project", type=str, default="yume-stage23", help="Wandb project name.")
