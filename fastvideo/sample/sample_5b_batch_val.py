...
def save_video(video):
    ...
    video = video_processor.postprocess_video(video.cpu().unsqueeze(0), output_type="pil")
    return video
...
def mp4_data(root_dir=None):
    ...
    for (video_path, videoid, keys, mouse) in vid_meta:
        try:
            caption = "This video depicts a first-person view (FPV) egocentric scene."
            vocab_k = {
...
MOVE_TO_KEY = {
    "forward": ("W", "Person moves forward (W)."),
    "backward": ("S", "Person moves backward (S)."),
    "left": ("A", "Person moves left (A)."),
    "right": ("D", "Person moves right (D)."),
}
ROT_TO_KEY = {
    "pans left": ("←", "Camera turns left (←)."),
    "pans right": ("→", "Camera turns right (→)."),
...
def _build_caption_line(move_phrase, rot_phrase, has_move, has_rot):
    ...
    return (
        f"{CAPTION_HEAD}{move_phrase}{rot_phrase}"
        f"Actual distance moved:{dist} at 100 meters per second. "
        f"Angular change rate (turn speed):{ang}. "
        f"View rotation speed:{rot_speed}."
    )

def _clause_to_segment(verb, direction):
    """Convert a single camera clause (verb, direction) into a caption line."""
    if verb == "moves":
        return _build_caption_line(MOVE_TO_KEY[direction][1], STILL_ROT, True, False)
    else:
...
def _get_static_line():
    ...
    if STATIC_LINE is None:
        STATIC_LINE = _build_caption_line(STILL_MOVE, STILL_ROT, False, False)
    return STATIC_LINE

def generate_per_sample_captions(tsv_prompt, total_steps=20):
    """Generate per-sample structured caption list from TSV prompt.
    ...
    static_line = _get_static_line()

    if not matches:
        return [static_line + action_text] * total_steps
    ...
    if not action_indices:
        mid = len(segments) // 2
        segments.insert(mid, static_line)
        action_indices.append(mid)

    n = len(segments)
    base = total_steps // n
    remainder = total_steps % n

    captions = []
    for i, seg_line in enumerate(segments):
        steps = base + (1 if i < remainder else 0)
        suffix = action_text if i in action_indices else ""
        for _ in range(steps):
            captions.append(seg_line + suffix)

    return captions

def load_tsv_dataset(tsv_path, first_frame_dir, num_samples_per_class=None, total_frames=33, H1=704, W1=1280):
    """Load TSV dataset with (action_class, prompt, video_name) rows.

    Reads pre-extracted first-frame images from first_frame_dir.
    Image filename derived from video_name: e.g. press_button_003.mp4 -> press_button_003.jpg
    ...
    for row in rows:
        ...
        img_name = os.path.splitext(video_name)[0] + '.jpg'
        img_path = os.path.join(first_frame_dir, img_name)

        if num_samples_per_class:
            class_counts.setdefault(action_class, 0)
            if class_counts[action_class] >= num_samples_per_class:
                continue

        if not os.path.isfile(img_path):
            print(f"Warning: first frame not found {img_path}, skipping")
            skipped += 1
            continue
        ...
        try:
            ...
            video_tensor[:, 0] = (resized_frame - 0.5) * 2
            sample_id = os.path.splitext(video_name)[0]
            dataset.append((video_tensor.permute(1, 0, 2, 3), sample_id, img_path, prompt))
        except Exception as e:
    ...
    main_print(f"  Loaded TSV dataset: {len(dataset)} samples, skipped {skipped}")
...
def sample_one(
    ...
    if image_sample and not t2v and skip_vlm:
        pixel_values_vid, videoid, img_path, tsv_prompt = dataset_ddp[index]
        ...
        if auto_caption:
            ...
            for i, c in enumerate(caption):
                if c not in seen:
                    seen.add(c)
                    main_print(f"    seg{len(seen)} [step {i}]: {c[:180]}...")
        elif strip_camera:
            action_prompt = strip_camera_clauses(tsv_prompt)
            caption = []
            with open(caption_path, 'r', encoding='utf-8') as file:
                for line in file:
                    caption.append(line.rstrip('\n') + action_prompt)
            main_print(f"  Caption (strip_camera, {len(caption)} steps): {caption[0][:150]}...")
        else:
            caption = []
            with open(caption_path, 'r', encoding='utf-8') as file:
                for line in file:
                    caption.append(line.rstrip('\n') + tsv_prompt)
            main_print(f"  Caption (skip_vlm, {len(caption)} steps): {caption[0][:150]}...")
        sample_num = len(caption)
        caption_ori = videoid
    elif image_sample and not t2v:
    ...
    for repeat_num in range(repeat_nums):
        ...
        if step % 1 == 0:
            ...
            for step_sample in range(sample_num):
                ...
                if step_sample == sample_num - 1 and auto_caption:
                    caption_file = os.path.splitext(filename)[0] + "_caption.txt"
                    with open(caption_file, 'w', encoding='utf-8') as cf:
                        for ci, cl in enumerate(caption):
                            cf.write(f"[step {ci:2d}] {cl}\n")
...
def main(args):
    ...
    if args.resume_from_checkpoint:
        if args.use_lora:
            lora_weights_path = os.path.join(args.resume_from_checkpoint, "adapter_model.bin")
            lora_weights = torch.load(lora_weights_path, map_location="cpu")
            set_peft_model_state_dict(transformer, lora_weights)
            init_steps = int(args.resume_from_checkpoint.split("-")[-1])
        else:
            (
                transformer,
                init_steps,
    ...
    dataset_ddp = None
    dataset_length = None
    if args.tsv_path and args.first_frame_dir:
        dataset_ddp, dataset_length = load_tsv_dataset(
            args.tsv_path,
            args.first_frame_dir,
        ...
        image_sample = True
    elif args.jpg_dir != None and not args.T2V:
    ...
    main_print(f"  Dataset length: {dataset_length}")
    ...
    for step in range(1, date_len):
        ...
        sample_one(
            ...
            num_autoregressive_steps = args.num_autoregressive_steps,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to TSV file with (action_class, prompt) pairs for batch validation.",
    )
    parser.add_argument(
        "--first_frame_dir",
        type=str,
        default=None,
        help="Directory containing pre-extracted first-frame jpg images.",
    )
    parser.add_argument(
        "--num_samples_per_class",
        type=int,
        default=None,
        help="Limit number of samples per action class (default: use all rows in TSV).",
    )
    parser.add_argument(
        "--skip_vlm",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=int,
        default=20,
        help="Number of autoregressive generation steps (each step = 32 pixel frames). Default 20 = ~40s at 16fps.",
    )
    parser.add_argument(
        "--strip_camera",
        action="store_true",
        default=False,
        help="Strip camera movement/rotation clauses from TSV prompt before appending to caption_re.txt prefix. "
             "Use this to avoid conflicting motion instructions between caption_re.txt and TSV prompt.",
    )
    parser.add_argument(
        "--auto_caption",
        action="store_true",
        default=False,
        help="Auto-generate per-sample structured captions from TSV prompt camera clauses. "
             "Each camera clause (moves/pans/tilts) becomes a separate segment; action becomes a static segment. "
             "Total steps divided evenly across segments. Overrides --strip_camera and --caption_path.",
    )
    parser.add_argument(
        "--auto_caption_steps",
        type=int,
        default=20,
        help="Total autoregressive steps when --auto_caption is enabled. "
             "Steps are divided evenly across detected segments. Default 20.",
    )
