...
def save_lora_checkpoint(transformer, optimizer, rank, output_dir, step):
    ...
    # Save LoRA adapter weights via FULL_STATE_DICT (rank0_only, no optimizer)
    with FSDP.state_dict_type(
            transformer,
            StateDictType.FULL_STATE_DICT,
            FullStateDictConfig(offload_to_cpu=True, rank0_only=True),
    ...
    if rank <= 0:
        ...
        lora_weights_path = os.path.join(save_dir, "adapter_model.bin")
    ...
    optimizer_dir = os.path.join(save_dir, "optimizer_state")
    os.makedirs(optimizer_dir, exist_ok=True)
    torch.save(optimizer.state_dict(),
               os.path.join(optimizer_dir, f"optimizer_rank{rank}.pt"))
...
def resume_lora_optimizer(transformer, checkpoint_dir, optimizer):
    ...
    optimizer_dir = os.path.join(checkpoint_dir, "optimizer_state")
    rank = torch.distributed.get_rank()
    per_rank_path = os.path.join(optimizer_dir, f"optimizer_rank{rank}.pt")

    if os.path.exists(per_rank_path):
        optimizer_state_dict = torch.load(per_rank_path, weights_only=False,
                                          map_location=f"cuda:{rank}")
        optimizer.load_state_dict(optimizer_state_dict)
    elif os.path.isdir(optimizer_dir):
        with FSDP.state_dict_type(transformer, StateDictType.SHARDED_STATE_DICT):
            optim_state = load_sharded_optimizer_state_dict(
            ...
            optim_state = optim_state["optimizer"]
            flattened_osd = FSDP.optim_state_dict_to_load(
                model=transformer, optim=optimizer, optim_state_dict=optim_state)
            optimizer.load_state_dict(flattened_osd)
    else:
